...
def get_job_data(job_page_link):
    ...
    job_title = job_header.find(
        'h1', 'jd-job-title').text.replace('\n', '').replace('\t', '').replace('\r', '').strip()
    company_name = job_header.find('h2').text.replace(
        '\n', '').replace('\t', '').replace('\r', '').strip()

    job_post_date_html = parsed_html.find(
        'ul', class_='top-job-insight').find_all('li')
    ...
    job_description = parsed_html.find(
        'div', class_='jd-desc job-description-main').text[16:].strip()

    job_skill_section = parsed_html.find(
        'div', class_='jd-sec job-skills clearfix')
    ...
    # Job basic info
    job_basic_info_list = parsed_html.find(
        'div', class_='job-basic-info').find('ul').find_all('li')
    job_basic_info = {}
    ...
    try:
        hiring_company_link = parsed_html.find(
            'ul', class_='hirng-comp-oth clearfix').find('span', class_='basic-info-dtl').text
    except:
    ...
    conn = db_connection()
    cursor = conn.cursor()
    sql = """INSERT INTO jobs (job_title, company_name, experience_required, salary, location, job_posted_on,job_post_link, job_description, job_skills, hiring_company_link, job_basic_info)  VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) """
    cursor.execute(sql, (job_title, company_name, experience_required, salary, location, job_post_date,
                   job_post_link, job_description, json.dumps(job_skills), hiring_company_link, json.dumps(job_basic_info)))
    conn.commit()
